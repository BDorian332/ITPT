...
from gui_vctk.core.correction.corrector_leaves import leaf_line_correction_step1, fix_leaves_step
from gui_vctk.core.correction.corrector_vote import (
...
def correction(
    ...
    nodes2, corners2, leaves2, _x_line = leaf_line_correction_step1(
        nodes,
        corners,
        leaves,
        img_width=img_width,
    ...
    leaves2, _rem, _add, _xref, _step = fix_leaves_step(leaves2, tol_px=5)

    # Étape 3: Application des règles de vote avec scores élevés
    scores = [2.5, 2.0]
    for score in scores:
        futur, _ = apply_rules_one_pass(nodes2, corners2, leaves2, delta=6)
        if printlog:
            print(f"------------- {score}")
            print_future_list(futur)

        nodes_to_add, corners_to_add, _modified = consolidate_future_and_select_points(
    ...
    scores = [1.5, 1.5]
    for score in scores:
        futur, _ = apply_rules_one_pass(nodes2, corners2, leaves2, r2c=0.5, delta=6)
        if printlog:
            print(f"------------- {score}")
            print_future_list(futur)
    ...
    nodes2, _modified_root = add_missing_root_node(nodes2, corners2)
